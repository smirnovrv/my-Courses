...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого строкаКоментария из Объект.КомментарииИФайлы Цикл 
		Если ЗначениеЗаполнено(строкаКоментария.Комментарий) Тогда
			сТело2=сТело2 + "<p>";
			сТело2=сТело2 + ОбщийМодульНаСервере.ПолучитьТекстHTMLДляСворачиваемогоТекста("Комментарий 	"
...
Процедура КомментарииИФайлыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "КомментарииИФайлыСсылкаНаФайл" Тогда
		ДобавитьФайлы();
	Иначе
...
Процедура СохранитьФайлВХранилище(Адрес,Владелец,Имя,номСтрокиТЧ)
	...
	СпрОбъект = Владелец.ПолучитьОбъект();
	текСтрока =  СпрОбъект.КомментарииИФайлы.Получить(номСтрокиТЧ);
	текСтрока.СсылкаНаФайл = НовФайл.Ссылка;
	СпрОбъект.Записать();
КонецПроцедуры 